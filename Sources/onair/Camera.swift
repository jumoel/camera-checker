...
class Camera: CustomStringConvertible {
	...
	func isOn() -> Bool {
		...
		if CMIOObjectGetPropertyDataSize(id, &STATUS_PA, 0, nil, &dataSize)
			== OSStatus(kCMIOHardwareNoError)
		{
	...
	func report() {
		...
		logger.info("\(self)")
		let props = [
			"kCMIODevicePropertyPlugIn": kCMIODevicePropertyPlugIn,
		...
		for (propName, prop) in props {
			...
			var (dataSize, dataUsed) = (UInt32(0), UInt32(0))
			if CMIOObjectGetPropertyDataSize(id, &pa, 0, nil, &dataSize)
				== OSStatus(kCMIOHardwareNoError)
			{
				if let data = malloc(Int(dataSize)) {
					CMIOObjectGetPropertyData(id, &pa, 0, nil, dataSize, &dataUsed, data)
					logger.info(
						"  \(propName): \(data.assumingMemoryBound(to: UInt8.self).pointee)")
				}
			}
		}
		logger.info("")
	...
	public var description: String {
		return "\(captureDevice.manufacturer)/\(captureDevice.localizedName)"
	}
}
